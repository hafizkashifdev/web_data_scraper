...
const baseUrl = 'https://developer.virginmoney.com';
...
// Function to read URLs, Page Names, and Class Mappings from Excel file
function readExcelFile(filePath) {
    try {
        ...
        return data.map((row, index) => ({
            pageName: row['Name']?.toString() || `Untitled_${index}`, // Use 'Name' column
            url: row['Link']?.toString() || '',                     // Use 'Link' column
            originalClass: row['Original Class']?.toString() || '',  // Add this
            newClass: row['New Class']?.toString() || ''       // Add this
...
(async () => {
    ...
    try {
        ...
        const excelFilePath = path.join(process.cwd(), 'extracted_links.xlsx');
        ...
        for (const { pageName, url, originalClass, newClass } of linksData) { // Get class names
            if (!isValidUrl(url)) {
            ...
            try {
                ...
                try {
                    ...
                    //  *** INJECT SCRIPT TO CHANGE CLASS NAMES  ***
                    await page.evaluate((originalClass, newClass) => {
